...
def addGUIContent():
    ...
    RXGainModeOptionMenu = apply(
        tk.OptionMenu,
        (top.RXGainModeFrame, gainMode) + tuple(PlutoController.rxGainModes))
    RXGainModeOptionMenu.pack()

    rxPlot = tk.StringVar()
    rxPlot.set(PlutoController.rxPlotList[PlutoController.rxPlotIndex])

    RXPlotOptionMenu = apply(
        tk.OptionMenu,
        (top.RXPlotFrame, rxPlot) + tuple(PlutoController.rxPlotList))
    RXPlotOptionMenu.pack()

    txPlot = tk.StringVar()
    txPlot.set(PlutoController.txPlotList[PlutoController.txPlotIndex])

    TXPlotOptionMenu = apply(
        tk.OptionMenu,
        (top.TXPlotFrame, txPlot) + tuple(PlutoController.txPlotList))
    ...
    popupMenu = tk.OptionMenu(
        top.ModulationSchemeFrame, modScheme,
        *PlutoController.ModulationFactory.SUPPORTED_SCHEMES)
    popupMenu.place(relx=0.5, rely=0.25)

    # Set Bindings for updates
    gainMode.trace(
        "w", lambda *args: PlutoController.updateGainMode(gainMode.get().upper(
        )))
    ...
    modScheme.trace(
        "w", lambda *args: PlutoController.updateModScheme(modScheme.get()))

    # add bindings for checkboxes
    PlutoConfig_support.rxImaginary.trace(
        'w', lambda *args: PlutoController.updateRXImaginary(
            PlutoConfig_support.rxImaginary.get()))
    PlutoConfig_support.txImaginary.trace(
        'w', lambda *args: PlutoController.updateTXImaginary(
            PlutoConfig_support.txImaginary.get()))
    PlutoConfig_support.txOn.trace(
        'w', lambda *args: PlutoController.setTXStatus(PlutoConfig_support.txOn
                                                       .get()))
...
def addMenubar(w):
    ...
    menubar.add_command(label="Throughput Test", command=testThroughput)
...
def startTicTacToe():
    global ttt_opened
    if not throughputPageOpened and not ttt_opened:
        ttt_opened = True
        ...
        launchTTT.startTicTacToe(root, playerNumber, onExit)


def testThroughput():
    global throughputPageOpened
    if not throughputPageOpened and not ttt_opened:
        throughputPageOpened = True
        PlutoController.setTXStatus('0')
        PlutoController.readRX = False

        def onExit():
            global throughputPageOpened
            PlutoController.setTXStatus('1')
            PlutoController.readRX = True
            throughputPageOpened = False

        throughputPage.start(root, onExit)


def update(evnt):

    # Update Back-end
    PlutoController.threadPeriod = float(
        PlutoConfig_support.threadUpdate.get())
    ...
    PlutoController.getSdr().rx_bandwidth = float(
        PlutoConfig_support.rxBandwidth.get())
    PlutoController.getSdr().rx_lo_freq = float(
        PlutoConfig_support.rxFrequency.get())
    ...
    PlutoController.getSdr().tx_lo_freq = float(
        PlutoConfig_support.txFrequency.get())
    PlutoController.getSdr().tx_bandwidth = float(
        PlutoConfig_support.txBandwidth.get())
    ...
    PlutoController.getSdr().sampling_frequency = P * desiredBandwidth / (
        1 + alpha)
...
def updateGUI():
    PlutoConfig_support.samplingFrequency.set(
        PlutoController.getSdr().sampling_frequency)
    ...
    PlutoConfig_support.rxDecimation.set(
        PlutoController.getSdr().rx_decimation)
    ...
    PlutoConfig_support.txInterpolation.set(
        PlutoController.getSdr().tx_interpolation)
    ...
    PlutoConfig_support.desiredBandwidth.set(
        PlutoController.getDesiredBandwidth())
